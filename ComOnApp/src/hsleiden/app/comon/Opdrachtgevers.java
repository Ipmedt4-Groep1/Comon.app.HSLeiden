...
public class Opdrachtgevers extends ListActivity {
	...
	// url to get all opdrachtgevers list
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Loading opdrachtgevers in Background Thread
		new LoadAllOpdrachtgevers().execute();

		//         Get listview
		ListView lv = getListView();

		// bij het kiezen van opdrachtgever
		// verwijst naar opdrachtgever detail pagina
	...
	/**
	 * Background Async Task to Load all opdrachtgevers by making HTTP Request
	 * */
	class LoadAllOpdrachtgevers extends AsyncTask<String, String, String> {
		...
		/**
		 * getting All opdrachtgevers from url
		 * */
		protected String doInBackground(String... args) {
			...
			try {
				...
				if (success == 1) {
					// opdrachtgevers gevonden
					// Getting Array of opdrachtgevers
					opdrachtgevers = json.getJSONArray(TAG_OPDRACHTGEVERS);

					// looping through All opdrachtgevers
		...
		protected void onPostExecute(String file_url) {
			// dismiss the dialog after getting all opdrachtgevers
			pDialog.dismiss();
